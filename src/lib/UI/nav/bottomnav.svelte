...
<div class="w-full h-fit bg-sky-500/10 dark:bg-white/5 mt-20 text-black dark:text-white/70 text-xs">
		<div class="max-w-[900px] h-fit p-1 pl-10 flex flex-row justify-between w-full mx-auto items-center">
			<a href="https://forms.gle/5dijSo4zbkeh2DDt6" target="_blank" ><i class="fa fa-exclamation-triangle mr-1" aria-hidden="true"/>Report</a>
            <button on:click|preventDefault={()=>{darkmodeswitch()}}>{#if !dark}
 ...
 <h1 class="px-2 py-1 bg-white/5 hover:bg-white/20 text-center rounded-md text-white">
                <i class="fa fa-sun" aria-hidden="true"/></h1>{/if}</button>
        </div>
	</div>