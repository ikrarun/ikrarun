<script lang="ts">
	 import data  from '$lib/json/sitedata.json';
...
<div id="root" class="min-h-screen flex flex-col justify-between relative">
	<main id="work" class=" relative w-screen text-black dark:text-white dark:bg-black bg-white items-center justify-center">
		<Header fix={true} />
		<div class="flex flex-col mt-12 space-y-2 items-start justify-center">
			<!-- Headings -->
			<div class="relative  max-w-[900px] mx-auto  sm:p-4 w-full">
				<h3 class="sm:text-3xl text-xl text-center sm:text-left font-bold">From Lines of Code to Digital Marvels</h3>
				<h3 class="text-xs sm:text-sm text-center sm:text-left text-gray-400 font-bold">
					A Showcase of My Web Projects and Innovations
				</h3>
			</div>
			...
			<div class="container  max-w-[900px] w-full mx-auto p-4">
				...
				>
					{#each pdata as item (item.id)}
						<div role="gridcell" tabindex="0" on:keydown={()=>{}} on:click|preventDefault={()=>{window.open(item.link,'_self')}}
						...
						>
							<img class="rounded-md mb-1" loading="lazy" src={item.image} alt="" />
							<h1 class="text-xl px-1 text-blue dark:text-gray-200 font-semibold">{item.title}</h1>
							<h1 class="text-xs text-gray-700/80 dark:text-gray-200/80 px-1 font-extralight">{item.description}</h1>
						</div>
					{/each}
				</div>
			</div>
